	...
	if( xTotalDataLength <= ( size_t ) ipMAX_UDP_PAYLOAD_LENGTH )
	{
			...
			( FreeRTOS_bind( xSocket, NULL, 0u ) == 0 ) )
		{
			...
			if( pxNetworkBuffer != NULL )
			{
				...
				UDPPacket_t *pxUDPPacket = ( UDPPacket_t * ) pxNetworkBuffer->pucEthernetBuffer;                                                                 //Navin
				eARPLookupResult_t eReturned;                                                                                                 //Navin
				eReturned = eARPGetCacheEntry( &( pxNetworkBuffer->ulIPAddress ), &( pxUDPPacket->xEthernetHeader.xDestinationAddress ) );    //Navin
				if(eReturned != eARPCacheHit) xTotalDataLength = 0;                                                                           //Navin
