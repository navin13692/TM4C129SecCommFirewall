	...
	if( eReturned == eProcessBuffer )
	{
		...
		switch( pxEthernetHeader->usFrameType )
		{
			case ipARP_FRAME_TYPE :
				...
				eReturned = eARPProcessPacket( ( ARPPacket_t * ) pxNetworkBuffer->pucEthernetBuffer, pxNetworkBuffer );
				break;
...
eFrameProcessingResult_t eReturn;
const IPHeader_t * pxIPHeader = &( pxIPPacket->xIPHeader );
UBaseType_t uxHeaderLength = ( UBaseType_t ) ( ( (pxIPHeader->ucVersionHeaderLength) & 0x0Fu ) << 2 );//Navin
uint8_t ucProtocol;
	...
	if( eReturn == eProcessBuffer )
	{
		if( uxHeaderLength > ipSIZE_OF_IPv4_HEADER )
		{
			...
			//pxNetworkBuffer->xDataLength =20;
		}
	...
	pxNetworkBuffer->xDataLength= FreeRTOS_htons(pxIPHeader->usLength);
...
EthernetHeader_t *pxEthernetHeader;
	...
	{
		pxEthernetHeader = ( EthernetHeader_t * ) ( pxNetworkBuffer->pucEthernetBuffer );
		//unsigned char frameType = pxEthernetHeader->usFrameType;
		...
		//uint32_t  ui32Loop;
	//for(ui32Loop=0;ui32Loop<ipMAC_ADDRESS_LENGTH_BYTES;ui32Loop++)
	//{
	//    *(&(pxEthernetHeader->xDestinationAddress) + ui32Loop) = *(&(pxEthernetHeader->xSourceAddress) + ui32Loop);
	//   *((uint8_t*)&(pxEthernetHeader->xSourceAddress) + ui32Loop) = xDefaultPartUDPPacketHeader.ucBytes[ui32Loop];
	//}
	//pxEthernetHeader->usFrameType=frameType;
	/* Send! */
